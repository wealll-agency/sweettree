                 ...
                 <div className="row g-2">
                      {combo.components?.map((comp, idx) => (
                         <div key={idx} className="col-12 col-sm-6">
                           <Link 
                             href={`/shop-details?${comp.name || comp.product?.name ? (comp.name || comp.product?.name).toString().trim().replace(/\s+/g, '-').replace(/[^\w\-]+/g, '').replace(/\-\-+/g, '-') : (comp.product?._id || comp.product)}`} 
