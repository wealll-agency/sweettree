...
export default function RefundDetailsModal({ refund, onClose }) {
  ...
  return createPortal(
    <div className="position-fixed top-0 start-0 w-100 h-100 d-flex justify-content-center align-items-center" style={{ backgroundColor: 'rgba(0,0,0,0.5)', zIndex: 1050, backdropFilter: 'blur(4px)' }} onClick={onClose}>
      <div className="card shadow-lg border-0 rounded-4" style={{ width: '800px', maxWidth: '95vw', maxHeight: '90vh', overflow: 'hidden' }} onClick={(e) => e.stopPropagation()}>
        ...
        <div className="card-header bg-white d-flex justify-content-between align-items-center border-bottom-0 pt-4 px-4">
          ...
          <button className="btn btn-sm btn-light rounded-circle p-2 d-flex align-items-center justify-content-center" onClick={onClose}>
        ...
        <div className="card-body px-4 pb-4" style={{ overflowY: 'auto' }}>
                        ...
                        <td className="ps-3 py-2">
                          <div className="d-flex align-items-center gap-2">
                            <div style={{ width: '30px', height: '30px', backgroundColor: '#f0f0f0', borderRadius: '4px', overflow: 'hidden' }}>
                              {item.product?.image && <Image src={`${process.env.NEXT_PUBLIC_API_URL ? process.env.NEXT_PUBLIC_API_URL.replace('/api', '') : 'https://www.sweettreeon.com'}${item.product.image}`} alt="" width={60} height={60} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />}
                            </div>
          ...
          {/* Footer / Actions inside body since card-footer can be problematic */}
          <div className="border-top pt-4 mt-4 d-flex justify-content-between bg-white">
