...
async function run() {
    ...
    // 1. SHA256 (hashText + secretKey) -> This is the most common for Indian PG
    await testHash("SHA256 (hashText + secretKey)", crypto.createHash('sha256').update(hashText + secureHashKey).digest('hex').toLowerCase());
    
    // 2. SHA256 (secretKey + hashText)
    await testHash("SHA256 (secretKey + hashText)", crypto.createHash('sha256').update(secureHashKey + hashText).digest('hex').toLowerCase());

    // 3. HMAC-SHA256 (this is what we currently have)
    await testHash("HMAC-SHA256", crypto.createHmac('sha256', Buffer.from(secureHashKey, 'utf8')).update(hashText).digest('hex').toLowerCase());
